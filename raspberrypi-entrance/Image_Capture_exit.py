...
def on_connect(client, userdata, flags, rc):
    ...
    client.subscribe("smart_park/exit_dist")
...
def image_getter( name, camera, rootpath='' ):
    ...
    while running:
        ...
        if(distance < 10):
            ...
            path = '/home/pi/repo/smart_park/raspberrypi-entrance/Temp_exit'
            ...
            client.publish("smart_park/exit_plate",plate_number[2:], retain=False)
            client.disconnect()
            client.loop_stop()
            
            client.connect(broker_address, 1883, 60)
            client.loop_start()    #start the loop
            client.publish("smart_park/exit_plate",plate_number, retain=False)
            client.disconnect()
            client.loop_stop()
            crp_path = path + '/Crop'
